...
public class BeautyServiceServiceImpl implements BeautyServiceService {
    ...
    @Transactional(readOnly = true)
    @Override
    public List<BeautyServiceResponseDto> getAllServices() {

        return beautyServiceRepository.findAllStream().map(s -> modelMapper.map(s, BeautyServiceResponseDto.class)).toList();
    }
